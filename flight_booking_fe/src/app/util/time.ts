...
export function transformDate(utcDate: any): string {
  ...
  return `${year}-${paddedMonth}-${paddedDay}T${paddedHour}:${paddedMinute}:00`;
}
...
export function convertToUserTimezone(
  ...
  console.log('utcData 1', utcDate);

  console.log(
    'utcDate: ', transformDate(utcDate)
  );

  return formatInTimeZone(
    transformDate(utcDate),
    userTimezone,
    format
  );
